...
function ListerFinancesPage() {
  ...
  const loadData = async (nextFilters = filters) => {
    ...
    try {
      const [summaryResponse, paymentsResponse] = await Promise.all([
        getListerFinanceSummary(nextFilters),
        getListerFinancePayments(nextFilters)
      ]);
      setSummary(summaryResponse || {});
      setRows(Array.isArray(paymentsResponse?.data) ? paymentsResponse.data : []);
      setPagination(paymentsResponse?.pagination || { page: 1, totalPages: 1, total: 0, limit: 20 });
    } catch (error) {
      notify(error.message || "Could not load finances data.", "danger");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleExportCsv = () => {
    const url = getListerFinanceCsvUrl(filters);
  ...
  return (
    <PortalLayout
      title="Finances"
      subtitle="Track your sponsorship spend, filter payments, and download platform receipts."
    >
      {/* KPI Cards */}
      <div className="kr-pricing-stat-row">
        <div className="kr-pricing-stat kr-pricing-stat--blue">
          ...
          <div>
            ...
            <p className="kr-pricing-stat-label">Total Spend (USD)</p>
          </div>
        </div>
        <div className="kr-pricing-stat kr-pricing-stat--green">
          ...
          <div>
            ...
            <p className="kr-pricing-stat-label">Total Spend (KES)</p>
          </div>
        </div>
            ...
            <div className="kr-filter-modal-body" style={{ display: "block", padding: "0" }}>
              <div className="kr-audit-modal-section">
                ...
                <div className="kr-audit-modal-grid">
                  ...
                  <div className="kr-audit-modal-field">
                    <label className="kr-settings-field-label" htmlFor="checkoutRef">Reference</label>
                    <input id="checkoutRef" name="checkoutRef" type="text" className="kr-form-input" value={filters.checkoutRef} onChange={handleFilterChange} placeholder="checkout ref" />
                  </div>
                </div>
              </div>
...
export default ListerFinancesPage;
