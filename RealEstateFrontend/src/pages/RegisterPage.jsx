...
function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      notify("Account created successfully. You can now log in to KenReal Estates.", "success");
      const returnTo = new URLSearchParams(location.search).get("returnTo");
      const safeReturnTo =
        returnTo && returnTo.startsWith("/") && !returnTo.startsWith("//") ? returnTo : null;
      navigate(safeReturnTo ? `/login?returnTo=${encodeURIComponent(safeReturnTo)}` : "/login");
    } catch (submitError) {
